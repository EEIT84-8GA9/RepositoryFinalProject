...
@WebFilter(
		urlPatterns = { "/_02_sellhouse/SellHouseUser.jsp" }, 
		initParams = { 
	@WebInitParam(name ="sellhouse", value ="/_02_sellhouse/SellHouseUser.jsp"), 			
		}
		)
public class SellHouseUserFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		requestURI  = req.getRequestURI();
			...
			//SellHouseBean bean=new SellHouseBean();
			 List<SellHouseBean> result =dao.select_user_account(user);
			System.out.println("ddddd"+result);
			req.setAttribute("select", result);
			request.getRequestDispatcher("/_02_sellhouse/SellHouseUser.jsp").forward(req, resp);
