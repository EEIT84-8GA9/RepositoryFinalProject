...
@WebServlet(urlPatterns = { "/pages/photo.view" }, initParams = { @WebInitParam(name = "defaultFile", value = "/img/gs.png") })
public class PhotoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (photoid != null && photoid.trim().length() != 0) {
			try {
				Furniture_id = Integer.parseInt(photoid.trim());
		...
		try {
			...
			stmt = conn
					.prepareStatement("select Furnitur_photo1_name,Furnitur_photo1 from Furniture where Furniture_id = ?");
			...
			/*
			 * response.setContentType("image/png"); OutputStream out =
			 * response.getOutputStream(); byte[] buffer = new byte[4*1024]; int
			 * len = photoInputStream.read(buffer); while(len!=-1) {
			 * out.write(buffer, 0, len); len = photoInputStream.read(buffer); }
			 */
